...
patient_options = df['email'].dropna().unique().tolist()
...
if selected_patient:
    df_patient = df[df["email"] == selected_patient].copy()
    st.write("Paciente: ", selected_patient)
    ...
    with col1:
        st.subheader("Café da Manhã")
        plt.bar(df['bia_cafe1'], df['ecg_cafe1'], color='teal')
    ...
    with col2:
        st.subheader("Almoço")
        plt.bar(df['bia_almoco1'], df['ecg_almoco1'], color='cyan')
        plt.xticks(rotation=45)
        st.pyplot(plt)
        plt.clf()  # Limpar a figura
    ...
    with col3:
        st.subheader("Jantar")
        plt.bar(df['bia_jantar1'], df['ecg_jantar1'], color='royalblue')
        plt.xticks(rotation=45)
        st.pyplot(plt)
        plt.clf()  # Limpar a figura
    ...
    with col6:
        st.subheader("Libre")
        plt.barh(df['email'], df['libre'], color='teal')
    ...
    with col7:
        ...
        plt.bar(df['diario_alimentar'], df['diario_alimentar'], color='turquoise')
        plt.xticks(rotation=45)
        st.pyplot(plt)
