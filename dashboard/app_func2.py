...
if selected_patient:
    ...
    with col1:
        st.subheader("Café da Manhã")
        # Posições no eixo X
        #selected_columns = ['bia_cafe1', 'ecg_cafe1', 'bia_cafe2', 'ecg_cafe2']
        selected_columns = ['bia_cafe1', 'ecg_cafe1', 'bia_cafe2', 'ecg_cafe2']
        dfq_selected = dfq[selected_columns]

        # Contar quantos 1s e 0s há nas colunas selecionadas
        count_1s = dfq_selected.sum(axis=0)  # Quantos 1s em cada coluna
        count_0s = dfq_selected.shape[0] - count_1s  # O restante são 0s

        # Posições no eixo X
        labels = dfq_selected.columns
    ...
    with col2:
        st.subheader("Almoço")
        plt.bar(dfq['bia_almoco1'], dfq['ecg_almoco1'], color='cyan')
    ...
    with col3:
        st.subheader("Jantar")
        plt.bar(dfq['bia_jantar1'], dfq['ecg_jantar1'], color='royalblue')
        plt.xticks(rotation=45)
        st.pyplot(plt)
        plt.clf()  # Limpar a figura
    ...
    with col4:
        ...
        ax1.pie(dfq['sono'].value_counts(), labels=dfq['sono'].unique(), autopct='%1.1f%%', startangle=90)
    ...
    with col5:
        ...
        fig1, ax1 = plt.subplots()
        ax1.pie(dfq['libre'].value_counts(), labels=dfq['libre'].unique(), autopct='%1.1f%%', startangle=90)
        centre_circle = plt.Circle((0, 0), 0.70, fc='white')
        fig1.gca().add_artist(centre_circle)
        ax1.axis('equal')
        st.pyplot(fig1)
    ...
    with col6:
        st.subheader("Libre")
        plt.barh(dfq['email'], dfq['libre'], color='teal')
    ...
    with col7:
        ...
        plt.bar(dfq['diario_alimentar'], dfq['diario_alimentar'], color='turquoise')
        plt.xticks(rotation=45)
        st.pyplot(plt)
