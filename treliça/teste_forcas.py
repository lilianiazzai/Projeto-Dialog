...
class Trelica:
    ...
    def calcular_forcas_nas_barras(self):
        ...
        #forças de ação e reação para as normais
        Fxa = 0
        Fye = ((f2 * l1) + (f3 * (l1+l2)) + (f4 * (l1+l2+l3)) + (f5 * (l1 + l2 + l3 + l4))) / (l1 + l2 + l3 + l4)
        ...
        if l8 > l6 and l8 > l10:
            ...
            Nb[7] = ( (-1) * f3) - (Nb[11] * (l8 - l6) / l12) - (Nb[12] * (l8 - l10) / l13)
        ...
        if l10 > l8 and l8 >= l6:
            ...
            Nb[11] = (Nb[4]*(math.sin(self.beta + math.cos(self.beta) * math.sin(self.gama) / math.cos(self.gama))) + f2*math.sin(self.gama)/math.cos(self.gama) + Nb[5] * math.sin(self.gama)/math.cos(self.gama)) / (math.cos(0) + math.sin(0).math.tan(self.gama))
            Nb[6] = (Nb[4] * math.sin(self.beta) - Nb[11]*math.cos(0))/math.sin(self.gama)
            Nb[12] = Nb[11]*(l2/l12)/math.cos(self.lambida - 1.54) 
            Nb[7] = -f3 + Nb[12]*math.sin(self.lambida - 1.54) - Nb[11]*(l8-l6)/l12
            Nb[8] = (-f4 - Nb[9] - Nb[12] * (l10-l8)/l13 - Nb[10]*l10/l11)*(l9/l10)
            #Fc = ((Nb[6] * l8 / l7) + Nb[7] + (Nb[8] * l8 / l9))

        for c in range (0, 13):
            print(f'Força normal na barra {c+1}= {Nb[c]};')

    def calcular_virtuais_nas_barras(self):
        l1 = self.comprimentos[0]
        l2 = self.comprimentos[1]
        l3 = self.comprimentos[2]
        l4 = self.comprimentos[3]
        ...
        #forças de ação e reação para as virtuais
        Fc = 1 #força total no eixo y no ponto c
        Fxa = 0
        Fye = Fc * (l1 + l2) / (l1 + l2 + l3 + l4)
        Fya = 1 - Fye
        ...
        # H > F & H > G
        if l8 > l6 and l8 > l10:
            Nb[6] = ((Nb[4] * (l6 / l5) * (l2/l12)) + (Nb[5] * l2 / l12) - (Nb[4] * (l1/l5) * ((l8-l6)/l12)))
            Nb[11] = ((Nb[6] * l6 / l7) + Nb[5] + (Nb[4] * l6 / l5))
            Nb[12] = (Nb[11] * l2 * l13) / (l12 * l3)
            Nb[7] = ((-1) * Nb[11] * ((l8-l6)/l12)) - (Nb[12] * ((l8-l10)/l13)) #(-1) * (Nb[11] * (l8 - l6) / l12) - (Nb[12] * (l8 - l10) / l13)
            Nb[8] = ((Nb[6] * l6 / l7) - (Nb[7] + Fc)) / (l10/l9)
            Fc = ((Nb[6] * l6 / l7) + Nb[7] + (Nb[8] * l10 / l9))
    ...
    def calcular_angulos(self):
        ...
        self.alfa = trelica.calcular_angulo_entre_barras(self.encontrar_barra("1"), self.encontrar_barra("5"))
        print(f'O angulo entre as barras b1 e b5 (alfa) é {self.alfa:.2f} radianos, {np.degrees(self.alfa):.2f} graus')

        self.gama = trelica.calcular_angulo_entre_barras(self.encontrar_barra("6"), self.encontrar_barra("7"))
        print(f'O angulo entre as barras b6 e b7 (gama) é {self.gama:.2f} radianos, {np.degrees(self.gama):.2f} graus')

        self.beta = trelica.calcular_angulo_entre_barras(self.encontrar_barra("5"), self.encontrar_barra("6"))
        print(f'O angulo entre as barras b5 e b6 (beta) é {self.beta:.2f} radianos, {np.degrees(self.beta):.2f} graus')

        ##

        self.lambida = trelica.calcular_angulo_entre_barras(self.encontrar_barra("13"), self.encontrar_barra("8"))
        print(f'O angulo entre as barras b13 e b8 (lambda) é {self.lambida:.2f} radianos, {np.degrees(self.lambida):.2f} graus')

        self.psi = trelica.calcular_angulo_entre_barras(self.encontrar_barra("12"), self.encontrar_barra("8"))
        print(f'O angulo entre as barras b12 e b8 (psi) é {self.psi:.2f} radianos, {np.degrees(self.psi):.2f} graus')

        ##

        self.omega = trelica.calcular_angulo_entre_barras(self.encontrar_barra("2"), self.encontrar_barra("7"))
        print(f'O angulo entre as barras b2 e b7 (omega) é {self.omega:.2f} radianos, {np.degrees(self.omega):.2f} graus')

        self.phi = trelica.calcular_angulo_entre_barras(self.encontrar_barra("7"), self.encontrar_barra("8"))
        print(f'O angulo entre as barras b7 e b8 (phi) é {self.phi:.2f} radianos, {np.degrees(self.phi):.2f} graus')

        self.sigma = trelica.calcular_angulo_entre_barras(self.encontrar_barra("8"), self.encontrar_barra("9"))
        print(f'O angulo entre as barras b8 e b9 (sigma) é {self.sigma:.2f} radianos, {np.degrees(self.sigma):.2f} graus')

        self.mi = trelica.calcular_angulo_entre_barras(self.encontrar_barra("9"), self.encontrar_barra("3"))
        print(f'O angulo entre as barras b9 e b3 (mi) é {self.mi:.2f} radianos, {np.degrees(self.mi):.2f} graus')

        ##

        self.alfa2 = trelica.calcular_angulo_entre_barras(self.encontrar_barra("11"), self.encontrar_barra("4"))
        print(f'O angulo entre as barras b11 e b4 (alfa2) é {self.alfa2:.2f} radianos, {np.degrees(self.alfa2):.2f} graus')

        self.gama2 = trelica.calcular_angulo_entre_barras(self.encontrar_barra("10"), self.encontrar_barra("9"))
        print(f'O angulo entre as barras b10 e b9 (gama2) é {self.gama2:.2f} radianos, {np.degrees(self.gama2):.2f} graus')

        self.beta2 = trelica.calcular_angulo_entre_barras(self.encontrar_barra("10"), self.encontrar_barra("11"))
        print(f'O angulo entre as barras b10 e b11  (beta)2 é {self.beta2:.2f} radianos, {np.degrees(self.beta2):.2f} graus')
...
if __name__== "__main__":
        ...
        l4=2
        l6=1
        l5=l7=l9=l11=l12=l13= pow(l1*l1+l6*l6,1/2)
        l8=2
        l10=1        

        A = trelica.adicionar_no(0,0) 
        B = trelica.adicionar_no(l1,0) #B
        C = trelica.adicionar_no(l1+l2,0) #C
        D = trelica.adicionar_no(l1+l2+l3,0) #D
        E = trelica.adicionar_no(l1+l2+l3+l4,0) #E
        F = trelica.adicionar_no(l1,l6) #F
        H = trelica.adicionar_no(l1+l2,l8) #H
        G = trelica.adicionar_no(l1+l2+l3,l10) #G

        b1 = trelica.adicionar_barra(A, B, l1, "1")
        b2 = trelica.adicionar_barra(B,C,l2, "2")
        b3 = trelica.adicionar_barra(C,D,l3, "3")
        b4 = trelica.adicionar_barra(D,E,l4, "4")
        b5 = trelica.adicionar_barra(A,F,l5, "5")
        b6 = trelica.adicionar_barra(F,B,l6, "6")
        b7 = trelica.adicionar_barra(F,C,l7, "7")
        b8 = trelica.adicionar_barra(H,C,l8, "8")
        b9 = trelica.adicionar_barra(C,G,l9, "9")
        b10 = trelica.adicionar_barra(G,D,l10, "10")
        b11 = trelica.adicionar_barra(G,E,l11, "11")
        b12 = trelica.adicionar_barra(F,H,l12, "12")
        b13 = trelica.adicionar_barra(H,G,l13, "13")

        trelica.comprimentos.append(l1)
        trelica.comprimentos.append(l2)
        trelica.comprimentos.append(l3)
        trelica.comprimentos.append(l4)
        trelica.comprimentos.append(l6)
        trelica.comprimentos.append(l8)
        trelica.comprimentos.append(l10)
        trelica.comprimentos_diagonais.append(l5)
        trelica.comprimentos_diagonais.append(l7)
        trelica.comprimentos_diagonais.append(l9)
        trelica.comprimentos_diagonais.append(l11)
        trelica.comprimentos_diagonais.append(l12)
        trelica.comprimentos_diagonais.append(l13)

        trelica.calcular_angulos()
        trelica.calcular_forcas_nas_barras()
        trelica.calcular_virtuais_nas_barras()